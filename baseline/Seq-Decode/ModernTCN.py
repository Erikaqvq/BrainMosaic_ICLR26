...
def _adjust_mask(mask, original_seq_len, output_seq_len, patch_stride, patch_size) -> torch.Tensor:
    if mask.dim() == 3:             
        mask = mask.any(dim=1)          
    patch_mask = torch.zeros(mask.shape[0], output_seq_len, device=mask.device, dtype=torch.bool)
    
    for i in range(output_seq_len):
        start_idx = i * patch_stride
        end_idx = min(start_idx + patch_size, original_seq_len)
        patch_has_mask = mask[:, start_idx:end_idx].any(dim=1)
        patch_mask[:, i] = patch_has_mask
...
class ModernTCN(nn.Module):
    def __init__(self,patch_size, patch_stride, stem_dimension, downsample_ratio, ffn_ratio, num_blocks, large_size, small_size, dims,
                 nvars, small_kernel_merged=False, backbone_dropout=0.1, head_dropout=0.1, use_multi_scale=True, revin=True, affine=True,
                 subtract_last=False, seq_len=512, c_in=7, individual=False, target_window=96, class_drop=0.,class_num = 10):
        ...
        self.revin = revin
        if self.revin: self.revin_layer = RevIN(stem_dimension, affine=affine, subtract_last=subtract_last)
        ...
        for stage_idx in range(self.num_stage):
            layer = Stage(ffn_ratio, num_blocks[stage_idx], large_size[stage_idx], small_size[stage_idx], dmodel=dims[stage_idx],
                           nvars=stem_dimension, small_kernel_merged=small_kernel_merged, drop=backbone_dropout)
            self.stages.append(layer)


              
        patch_num = seq_len // patch_stride
        self.n_vars = stem_dimension
        self.individual = individual
    ...
    def _calculate_max_layers(self, n_timesteps):
        """Compute the maximum allowed number of pooling layers."""
        max_layers = 0
...
class ModernTCNEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.mix_vars = nn.Conv2d(
            in_channels=stem_dimension, out_channels=1,
    ...
    def forward(self, x: torch.Tensor, mask=None) -> torch.Tensor:
        ...
        """
        expect_channel = min(x.shape[1], x.shape[-1])
        x = _ensure_channels_first(x, expect_channel)
        B, C, T = x.shape
        feats = self._feature_only(x)                               

                                                                     
        x_mixed = feats.mean(dim=1, keepdim=True)

        x_seq = einops.rearrange(x_mixed, 'b 1 d n -> b n d')
        if mask is not None:
            patch_stride = self.backbone.patch_stride
            patch_size = self.backbone.patch_size
            mask = _adjust_mask(mask, T, x_seq.shape[1], patch_stride, patch_size)
