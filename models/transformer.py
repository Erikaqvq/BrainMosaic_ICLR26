...
class Transformer(nn.Module):
    def __init__(self, d_model=32, nhead=8,
    ...
    def forward(self, src, mask, query_embed, pos_embed=None):
        ...
        if pos_embed is not None:
            if pos_embed.shape[1] != N:
                pos_embed = pos_embed[:, :N, :] if pos_embed.shape[1] > N else\
                            F.pad(pos_embed, (0, 0, 0, N - pos_embed.shape[1]))
            pos_embed = pos_embed.to(dtype=src.dtype, device=src.device)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_pre(self, tgt, memory,
        ...
        tgt2 = self.self_attn(q, k, value=tgt2, attn_mask=tgt_mask,
                              key_padding_mask=tgt_key_padding_mask)[0]
        tgt = tgt + self.dropout1(tgt2)
        tgt2 = self.multihead_attn(query=self.with_pos_embed(self.norm2(tgt), query_pos),
                                   key=self.with_pos_embed(memory, pos),
                                   value=memory, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask)[0]
        tgt = tgt + self.dropout2(tgt2)
